...
useGoogleAuth();
const { loginFacebook, fakeLoginFacebook } = useFacebookAuth();
...
<template>
  <div class="mx-auto w-fit max-w-screen space-y-4 rounded bg-white p-6 shadow-md">
    ...
    <div class="flex items-center justify-between">
      ...
      <button type="button" class="ml-4 cursor-pointer rounded bg-blue-600 px-4 py-2 text-white disabled:opacity-50" @click="loginFacebook" :disabled="!!user?.facebook">Facebook Login</button>
      <button type="button" v-if="isFacebookFakeLogin" class="ml-4 cursor-pointer rounded bg-blue-600 px-4 py-2 text-white disabled:opacity-50" @click="fakeLoginFacebook" :disabled="!!user?.facebook">
        Facebook Fake Login
      </button>
    </div>
